...
import { Activity, AlertTriangle, Bot, Check, Columns3, Database, Play, RefreshCw, Search, ShieldCheck, Sparkles, Target, Waves, X } from "lucide-react";
import { API_URL, api } from "./api";
import type { AttackCard, Iteration, Provider } from "./types";

type View = "overview" | "catalog" | "loop" | "evaluation" | "portfolio" | "providers";
const nav: { id: View; label: string; icon: typeof Activity }[] = [
  ...
  { id: "evaluation", label: "Evaluation lab", icon: Target },
...
export default function App() {
  ...
  const content = view === "overview" ? <Overview cards={catalog} buckets={buckets} latest={latest} onRun={async () => {
    ...
    view === "evaluation" ? <EvaluationLab iterations={iterations} onNotice={setMessage} onOpenLoop={() => setView("loop")} /> :
...
type EvaluationBucket = { name: string; attack_card_count: number };
type DifficultyProfile = { name: string; label: string; description: string };
type EvaluationOptions = { buckets: EvaluationBucket[]; difficulty_profiles: DifficultyProfile[]; defaults: { seeds: number[]; scenarios_per_card: number; benign_count: number; realism_profile: string } };
type CoverageCell = { difficulty_profile: string; difficulty_label: string; bucket: string; record_count: number; fraud_record_count: number; miss_count: number; metrics: Record<string, number>; status: "strong" | "monitor" | "weak"; missed_subtypes: { subtype: string; miss_count: number }[]; representative_misses: { transaction_id: string; attack_subtype: string; fraud_score: number; reason_codes: string[] }[] };
type EvaluationCampaign = { campaign_id: string; created_at: string; source_iteration_id: string; configuration: { buckets: string[]; attack_card_count: number; difficulty_profiles: DifficultyProfile[]; seeds: number[]; scenarios_per_card: number; benign_count_per_arm: number; realism_profile: string; arm_count: number }; coverage_matrix: CoverageCell[]; summary: { cell_count: number; strong_cells: number; monitor_cells: number; weak_cells: number; lowest_recall_cell?: CoverageCell; follow_up_candidates: CoverageCell[] }; disclosure: string };
type EvaluationCampaignListItem = Pick<EvaluationCampaign, "campaign_id" | "created_at" | "source_iteration_id" | "summary" | "configuration">;

function EvaluationLab({ iterations, onNotice, onOpenLoop }: { iterations: Iteration[]; onNotice: (message: string) => void; onOpenLoop: () => void }) {
  const [options, setOptions] = useState<EvaluationOptions | null>(null);
  const [campaigns, setCampaigns] = useState<EvaluationCampaignListItem[]>([]);
  ...
  const [settings, setSettings] = useState({ source_iteration_id: iterations.at(-1)?.iteration_id ?? "", buckets: [] as string[], profiles: [] as string[], seeds: "101, 202", scenarios_per_card: "1", benign_count: "100" });

  async function loadCampaigns(preferredId?: string) {
    try {
      const response = await api<{ campaigns: EvaluationCampaignListItem[] }>("/evaluation-lab/campaigns");
      setCampaigns(response.campaigns);
      const selected = preferredId ?? selectedCampaignId ?? response.campaigns[0]?.campaign_id;
      if (selected) { setSelectedCampaignId(selected); await loadCampaign(selected); }
    } catch (error) { onNotice(error instanceof Error ? error.message : "Could not load evaluation campaigns."); }
  }
  async function loadCampaign(campaignId: string) {
    try { const next = await api<EvaluationCampaign>(`/evaluation-lab/campaigns/${campaignId}`); setCampaign(next); setSelectedCell(null); }
    catch (error) { onNotice(error instanceof Error ? error.message : "Could not load campaign evidence."); }
  }
  useEffect(() => {
    void api<EvaluationOptions>("/evaluation-lab/options").then((next) => {
      setOptions(next);
      setSettings((current) => ({ ...current, buckets: current.buckets.length ? current.buckets : next.buckets.map((item) => item.name), profiles: current.profiles.length ? current.profiles : next.difficulty_profiles.map((item) => item.name), seeds: current.seeds || next.defaults.seeds.join(", "), scenarios_per_card: current.scenarios_per_card || String(next.defaults.scenarios_per_card), benign_count: current.benign_count || String(next.defaults.benign_count) }));
    }).catch((error) => onNotice(error instanceof Error ? error.message : "Could not load Evaluation Lab options."));
    void loadCampaigns();
  }, []);
  useEffect(() => { if (!settings.source_iteration_id && iterations.length) setSettings((current) => ({ ...current, source_iteration_id: iterations.at(-1)?.iteration_id ?? "" })); }, [iterations, settings.source_iteration_id]);

  function toggleValue(key: "buckets" | "profiles", value: string) {
    setSettings((current) => ({ ...current, [key]: current[key].includes(value) ? current[key].filter((item) => item !== value) : [...current[key], value] }));
  }
  async function runCampaign() {
    if (!settings.source_iteration_id) { onNotice("Complete or select a source iteration with a frozen detector first."); return; }
    if (!settings.buckets.length || !settings.profiles.length) { onNotice("Select at least one fraud family and one difficulty profile."); return; }
    const seeds = settings.seeds.split(",").map((value) => Number(value.trim())).filter((value) => Number.isInteger(value) && value > 0);
    if (!seeds.length) { onNotice("Enter at least one positive seed, separated by commas."); return; }
    setBusy(true);
    try {
      const response = await api<{ campaign: EvaluationCampaign }>("/evaluation-lab/campaigns", { method: "POST", body: JSON.stringify({ source_iteration_id: settings.source_iteration_id, buckets: settings.buckets, difficulty_profiles: settings.profiles, seeds, scenarios_per_card: Number(settings.scenarios_per_card), benign_count: Number(settings.benign_count), realism_profile: "overlap" }) });
      setCampaign(response.campaign); setSelectedCampaignId(response.campaign.campaign_id); setSelectedCell(null); await loadCampaigns(response.campaign.campaign_id); onNotice(`Campaign ${response.campaign.campaign_id} completed. ${response.campaign.summary.monitor_cells + response.campaign.summary.weak_cells} cell(s) need follow-up.`);
    } catch (error) { onNotice(error instanceof Error ? error.message : "Evaluation campaign failed."); }
    finally { setBusy(false); }
  }
  const campaignProfiles = campaign?.configuration.difficulty_profiles ?? [];
  const campaignBuckets = campaign?.configuration.buckets ?? [];
  const cellAt = (bucket: string, profile: string) => campaign?.coverage_matrix.find((cell) => cell.bucket === bucket && cell.difficulty_profile === profile);
  return <section className="evaluation-workspace">
    <article className="panel evaluation-intro"><div><p className="eyebrow accent">ADVERSARIAL EVALUATION LAB</p><h3>Map defence coverage before the next mutation</h3><p>Systematically stress a frozen detector across payment-fraud families and bounded difficulty profiles. LLM row review is disabled so every cell is directly comparable.</p></div><Target size={38}/></article>
    <article className="panel evaluation-controls"><div className="panel-title"><div><h3>Configure a campaign</h3><p>Each profile × seed pair is a deterministic synthetic evaluation arm.</p></div><span>{settings.buckets.length} families</span></div><div className="evaluation-form"><label>Frozen detector source<select value={settings.source_iteration_id} onChange={(event) => setSettings({ ...settings, source_iteration_id: event.target.value })}><option value="">Select an iteration</option>{[...iterations].reverse().map((iteration) => <option key={iteration.iteration_id} value={iteration.iteration_id}>{iteration.iteration_id} · F1 {metric(iteration.evaluation_overall.f1)}</option>)}</select></label><label>Seeds (comma separated)<input value={settings.seeds} onChange={(event) => setSettings({ ...settings, seeds: event.target.value })}/></label><label>Scenarios / card<input type="number" min="1" max="10" value={settings.scenarios_per_card} onChange={(event) => setSettings({ ...settings, scenarios_per_card: event.target.value })}/></label><label>Benign records / arm<input type="number" min="20" value={settings.benign_count} onChange={(event) => setSettings({ ...settings, benign_count: event.target.value })}/></label></div><div className="evaluation-selectors"><div><small>Fraud families</small><div className="selector-chips">{options?.buckets.map((bucket) => <button key={bucket.name} className={settings.buckets.includes(bucket.name) ? "selector-chip selected" : "selector-chip"} onClick={() => toggleValue("buckets", bucket.name)}>{prettify(bucket.name)} <b>{bucket.attack_card_count}</b></button>)}</div></div><div><small>Difficulty profiles</small><div className="selector-chips">{options?.difficulty_profiles.map((profile) => <button key={profile.name} title={profile.description} className={settings.profiles.includes(profile.name) ? "selector-chip selected" : "selector-chip"} onClick={() => toggleValue("profiles", profile.name)}>{profile.label}</button>)}</div></div></div><div className="evaluation-action"><button className="primary-button" disabled={busy} onClick={() => void runCampaign()}><Play size={15}/>{busy ? "Running campaign…" : "Run adversarial sweep"}</button><small>Frozen detector · synthetic data only · no LLM row review</small></div></article>
    <article className="panel campaign-browser"><div className="panel-title"><div><h3>Saved campaigns</h3><p>Select reproducible evidence from a prior sweep.</p></div>{campaign && <span>{campaign.summary.cell_count} cells</span>}</div>{campaigns.length ? <select value={selectedCampaignId} onChange={(event) => { setSelectedCampaignId(event.target.value); void loadCampaign(event.target.value); }}>{campaigns.map((item) => <option key={item.campaign_id} value={item.campaign_id}>{item.campaign_id} · {item.source_iteration_id} · {item.summary.monitor_cells + item.summary.weak_cells} follow-up</option>)}</select> : <Empty text="No campaigns yet. Configure and run a deterministic sweep above."/>}</article>
    {campaign && <><article className="panel evaluation-summary"><div className="panel-title"><div><h3>{campaign.campaign_id} coverage matrix</h3><p>{campaign.configuration.attack_card_count} cards · {campaign.configuration.arm_count} deterministic arms · source {campaign.source_iteration_id}</p></div><span>{campaign.summary.monitor_cells + campaign.summary.weak_cells} follow-up</span></div><div className="evaluation-stat-grid"><Stat label="Strong" value={String(campaign.summary.strong_cells)} detail="Cells above demo thresholds"/><Stat label="Monitor" value={String(campaign.summary.monitor_cells)} detail="Inspect before next mutation"/><Stat label="Weak" value={String(campaign.summary.weak_cells)} detail="Prioritize red-team review"/><Stat label="Profiles" value={String(campaignProfiles.length)} detail={`${campaign.configuration.seeds.length} seeded repetitions`}/></div><div className="coverage-scroll"><div className="coverage-matrix" style={{ gridTemplateColumns: `minmax(190px, 1.1fr) repeat(${campaignProfiles.length}, minmax(160px, 1fr))` }}><div className="coverage-head">Fraud family</div>{campaignProfiles.map((profile) => <div className="coverage-head" key={profile.name} title={profile.description}>{profile.label}</div>)}{campaignBuckets.flatMap((bucket) => [<div className="coverage-label" key={`${bucket}-label`}>{prettify(bucket)}</div>, ...campaignProfiles.map((profile) => { const cell = cellAt(bucket, profile.name); return <button key={`${bucket}-${profile.name}`} className={`coverage-cell ${cell?.status ?? ""}`} onClick={() => cell && setSelectedCell(cell)} disabled={!cell}>{cell ? <><strong>{metric(cell.metrics.recall)}</strong><span>Recall · F1 {metric(cell.metrics.f1)}</span><small>{cell.miss_count} miss{cell.miss_count === 1 ? "" : "es"} · {cell.status}</small></> : "—"}</button>; })])}</div></div><p className="impact-disclosure">{campaign.disclosure}</p></article>{selectedCell && <article className="panel evaluation-evidence"><div className="panel-title"><div><p className="eyebrow accent">CELL EVIDENCE</p><h3>{prettify(selectedCell.bucket)} · {selectedCell.difficulty_label}</h3><p>{selectedCell.fraud_record_count} fraud records · {selectedCell.record_count} records including benign context</p></div><span className={`coverage-status ${selectedCell.status}`}>{selectedCell.status}</span></div><div className="impact-metrics"><div><small>Recall</small><strong>{metric(selectedCell.metrics.recall)}</strong></div><div><small>F1</small><strong>{metric(selectedCell.metrics.f1)}</strong></div><div><small>False-positive rate</small><strong>{metric(selectedCell.metrics.false_positive_rate)}</strong></div><div><small>Misses</small><strong>{selectedCell.miss_count}</strong></div></div><div className="evidence-columns"><section><h4>Missed subtypes</h4>{selectedCell.missed_subtypes.length ? selectedCell.missed_subtypes.map((item) => <div className="evidence-row" key={item.subtype}><span>{prettify(item.subtype)}</span><b>{item.miss_count} miss{item.miss_count === 1 ? "" : "es"}</b></div>) : <p className="muted">No missed fraud rows in this cell.</p>}</section><section><h4>Representative misses</h4>{selectedCell.representative_misses.length ? selectedCell.representative_misses.map((item) => <div className="representative-miss" key={item.transaction_id}><strong>{item.transaction_id}</strong><span>{prettify(item.attack_subtype)} · score {metric(item.fraud_score)}</span><small>{item.reason_codes.join(" · ") || "No reason codes recorded"}</small></div>) : <p className="muted">No representative misses are available for this cell.</p>}</section></div>{selectedCell.status !== "strong" && <div className="evaluation-followup"><div><strong>Suggested next step</strong><p>Inspect this family in the closed-loop review queue, then create a human-governed mutation only after reviewing its evidence.</p></div><button className="ghost-button" onClick={onOpenLoop}>Open closed loop</button></div>}</article>}</>}
  </section>;
}
